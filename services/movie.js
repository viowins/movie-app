...
const fetchMediaApi = async (pathname, query = '') => {
  ...
  const res = await fetch(`${process.env.MOVIE_DB_URL}${pathname}?${query}`, options);
...
export { fetchMediaApi, getTrendings, getHeroMovie, getMediaDetail, getMeidaVideos, getMediaRelaseDate, getMediaDirector }