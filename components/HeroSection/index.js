...
export default function HeroSection({ movie = {}, isLinked = false, mediaType, id, video = {}, }) {
  return (
    <div className={cn("relative aspect-3/2 md:aspect-25/9 flex flex-col justify-center rounded-lg px-8 py-4 overflow-hidden !bg-cover !bg-center !bg-no-repeat before:absolute before:left-0 before:w-2/3 before:h-full before:bg-gradient-to-r before:from-black before:from-30% before:to-transparent", !movie.backdrop_path &&  'bg-red-900/25')} style={{background: movie.backdrop_path && `url('https://image.tmdb.org/t/p/original${movie.backdrop_path}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat'}}>
      ...
      <div className='flex flex-col gap-3 max-w-5xl z-30'>
        <h1 className="text-lg sm:text-xl md:text-3xl lg:text-5xl font-semibold">{movie.original_title && movie.original_title}{movie.original_name && movie.original_name}</h1>
